...
$('#filter_by').on('change', function () {
    ...
    if($("#filter_by option:selected").text() === "Morosos" || $("#filter_by option:selected").text() === "Inactivo" || $("#filter_by option:selected").text() === "Saneados"  || $("#filter_by option:selected").text() === "Dellalles Saneados" || $("#filter_by option:selected").text() ==="Cartera General" || $("#filter_by option:selected").text() === "Saldos de Cartera" || $("#filter_by option:selected").text() === "Cartera Total" || $("#filter_by option:selected").text() === "Ultimo Abono" || $("#filter_by option:selected").text() === "CPPDDIASEMANA" || $("#filter_by option:selected").text() === "Moras acumulada"  || $("#filter_by option:selected").text() === "Prestamos Activos"  || $("#filter_by option:selected").text() === "Activos" || $("#filter_by option:selected").text() === "Vencidos" || $("#filter_by option:selected").text() === "Cancelados" || $("#filter_by option:selected").text() === "<= 20%"  ){
...
/***"<option value=\"Prestamos Activos\">Prestamos Activos</option>\n"+
    ...
    if ($("#groud_by option:selected").text() === "Gestores") {
        $('#filter_by').html("<option value=\"0\">-Seleccione-</option>\n" +
                             "<option value=\"Prestamos NoAbonado\">Prestamos No Abonado</option>\n"+
                            "<option value=\"Recaudado_sup\">Recaudado</option>\n"+
                            "<option value=\"Recaudado Clientes\">Recaudado Clientes</option>\n"
                            );
    }
...
/*<option value=\"0\">-Seleccione-</option>\n" +
    ...
    if ($("#groud_by option:selected").text() === "Prestamos") {
        $('#filter_by').html("<option value=\"Primera_cuota\">Primera Cuota</option>\n"+
                             "<option value=\"Moroso_p\">Morosos</option>\n" +
                            "<option value=\"cartera_saneada\">Saneados</option>\n"+
                            "<option value=\"Vencidos\">Vencidos</option>\n"+
                             "<option value=\"detalle_saneada\">Dellalles Saneados</option>\n"+

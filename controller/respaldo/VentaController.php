...
class VentaController extends ControladorBase
{
    ...
    if(!empty($params['search']['value']) ) {
            ...
            $busqueda = trim($params['search']['value']);
            $borrado = '';

            if ($busqueda !== '' && stripos(strtolower('activo'), $busqueda) !== false)
                $borrado = ' OR borrado = 0';
            if ($busqueda !== '' && stripos(strtolower('borrado'), $busqueda) !== false)
                $borrado = ' OR borrado = 1';
